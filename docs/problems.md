...
旧版本中，全文搜索在每次查询前要对全部持久化会话做一次整体校验（上游基础引擎至今如此）。只要**任意一个**会话文件里存在当前版本的冻结解码器无法接受的事件（已知实例：0.1.5 发布前 alpha 时代的构建把 `subagent/descriptor` 以版本 2 写进了 v0 代会话文件，而发布版 v0 解码器只接受版本 3），整个校验就会失败，导致**所有**跨会话搜索整体不可用——同一工作区里可能存在**大量**此类历史会话。精确读取与谱系工具不经过整体校验，近期会话（v3 代）也不受影响，因此问题常被误判为局部故障。此外，若插件与宿主各自解析到两份物理不同的会话查询包（版本号相同但模块实例不同），类型化错误会在边界处退化为上述通用文案，真实诊断只进日志。

### 解决

本 bundle 已内置修复：dsh-code 自带的容错会话检索引擎在校验时**跳过**无法解析的会话（每次跳过在日志中留一条警告），其余会话照常索引；被跳过的会话在后续搜索中自动重试，升级到能够解析它的 dsh 版本线后会自动回到索引，无需任何手动处理。旧版 dsh-code 的临时办法是把对应会话目录移出会话根目录隔离（移出后该会话不再出现在 resume 列表，移回即恢复）：
...
## 同一会话在另一个终端里打开时，`/delete` 拒绝删除

### 现象

终端 A 正在运行会话 X，终端 B 尝试用 `/delete` 删除 X（或包含 X 的父会话子树）时，命令提示该会话已在本进程或另一进程中打开，不会删除任何文件。

### 原因

`/resume` 列表中的 live 标记主要反映当前进程的观察结果，跨进程状态不一定能及时显示。删除操作因此不依赖这个显示：它会在接触任何文件之前，通过持久化服务为整个删除子树逐一申请写句柄；JSONL 后端的 `session.lock` 内核锁会拒绝另一进程已占用的会话。只要任一成员无法加锁，之前取得的锁会全部释放，整个删除计划保持不变。

### 解决

- 先在占用会话的终端中退出或切换到其他会话，再重试 `/delete`。
- 如果仍然拒绝，请保留原始会话文件，并检查提示中的持久化或锁错误；系统会按“无法证明安全就不删除”的原则停止。
- 较旧版本只检查本进程的 live 状态，可能误删另一个终端仍在写入的会话；请升级到包含跨进程删除锁保护的版本。
