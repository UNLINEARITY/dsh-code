...
export default defineConfig({
  entry: ['src/index.ts', 'src/startup.ts', 'src/invariant.ts', 'src/session-query.ts'],
  outDir: 'lib',
  format: 'esm',
  target: 'es2024',
  ...
  deps: {
    ...
    // The transitive closure is intentionally bundled with those entry
    // packages; suppress the allowlist hint rather than maintaining a brittle
    // duplicate list of Ink's implementation dependencies.
    onlyBundle: false,
  },
})
